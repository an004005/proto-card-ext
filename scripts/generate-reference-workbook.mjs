...
function buildWorkbook() {
  ...
  const rules = [
    ...
    ['맵', '구역 추첨', `${FACILITY.RUN_SECTOR_COUNT}구역 / 전체 ${FACILITY.ALL_SECTOR_IDS.length}구역`, `런 시작에 시드로 뽑는다. 입구·관리동은 항상 포함되고 나머지 ${FACILITY.RUN_SECTOR_COUNT - 1}개를 균등 추첨. 동력·정비동이 뽑히면 링에서 입구 정반대에 놓인다`, 'src/engine/facilityGraph.js'],
