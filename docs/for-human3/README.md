...
| 5. 실패 조건 | 체력 0 또는 시설 시각 240칸 도달. 계약 미완수는 탈출 실패와 별개이며 점수 불이익이다 |
...
| 단계 | 플레이어의 고민 |
|---|---|
| 계약 선택 | 짐을 운반할 것인가, 폭약을 설치하고 빠질 것인가, 데이터를 얻어 다른 구역까지 갈 것인가? 계약 카드마다 이번 시설의 네 구역이 미리 적혀 있으므로, 어느 시설을 볼지도 여기서 고른다 |
...
| 탈출 | 탈출구 가동을 언제 걸고, 게이지를 채우는 동안 다가오는 적을 어떻게 넘길까? |
...
매번 세 유형에서 하나씩 계약을 제안한다. 제안마다 그 계약을 골랐을 때 지어질 네 구역이 링 순서로 함께 보인다. 하나를 수락하면 선불 재화가 가방에 들어오고 목표 위치가 공개되며, 화면이 보여준 그 네 구역이 그대로 시설이 된다.
...
- 현재 방은 직접 확인한다. 인접한 곳의 무료 관측은 적의 존재·무리 수·행동 모드와 "무언가 놓여 있다"까지 알려 준다. 등급·역할축·남은 횟수는 알려 주지 않는다. 지각 2 이상이면 그 시야가 한 홉 더 뻗고 대기 중에도 끊기지 않는다.
- 모든 카메라의 위치와 상태는 출격 시작부터 지도에 보인다.
- 기본 정찰은 2칸을 쓴다. 지각이 높으면 같은 정찰로 적의 경계·다음 이동·편성 등 더 깊은 정보를 얻는다. 지각 3부터 정찰 범위도 넓어진다.
...
**다른 시스템과의 관계:** 장비가 정보의 깊이와 사거리를 바꾼다. 이동은 어느 통로든 1칸이므로 거리는 곧 지나야 할 방의 수다. 출구 마감이 정보에 투자할 수 있는 시간을 제한한다.
...
카메라 감지, 시체·강한 흔적 발견, 허탕 조사 등은 경계 압력을 쌓는다. 압력 10마다 경계가 한 단계 오르며 최고 3단계다. 3단계가 되면 그 구역이 **추적자** 하나를 내보낸다. 순찰 경로가 없고 나를 향해 걸으며, 20칸 동안 놓치거나 경계가 3 아래로 내려가거나 통제실을 장악해야 물러난다.

카메라는 지나가는 것만으로는 발각하지 않는다. 그 방에서 작업을 마치거나 그 방을 떠날 때의 은신, 그리고 그 방에서 난 소음이 판정을 부른다. 소음은 은신과 무관하게 즉시 발각이다.
...
- 전투 한 라운드에 시설 시간 1칸이 흐른다. 도중 승리해도 그 라운드 비용은 든다. 적 기습은 추가 선공 구간 1칸이 붙는다.
...
| 현장 교체 | 교체 한 건에 시간 2칸이 든다. 새 장비는 다음 전투의 카드와 현장 능력을 바꾼다 |
| 부상 | 체력이 절반 이하면 여섯 침투 능력이 전부 1, 4분의 1 이하면 2 내려간다. 회복하면 그 자리에서 풀린다 |
...
**다른 시스템과의 관계:** 회수 계약의 짐이 손패를 방해할 수 있다. 기동 변화는 고지대 지름길이 열리는지를, 은신 변화는 전투 시작 조건을 바꾼다. 체력이 줄면 여섯 능력이 함께 내려가므로 회복품이 곧 침투 자원이다.
...
| 가동 | 출구 현장에서 1칸을 써서 건다. 가동 신호가 적을 끈다 |
| 개방까지 | 가동 1칸을 포함한 게이지 전체가 해킹 능력에 따라 5~2칸이다. 그 자리에서 대기로 채워야 하며 자리를 뜨면 가동이 취소된다 |
| 열린 시간 | 5칸. 열려 있는 동안 출구에 있어야 한다. 개방 시간을 놓쳤다면 남은 마감 안에서 재요청을 판단한다 |
| 일반 출구 가동 마감 | 시설 시각 150칸부터 새 가동이 불가능하다. 그 전에 시작한 게이지와 열린 시간은 끝까지 유지된다. 문이 닫힌 상태로 마감을 넘기면 영구 폐쇄된다 |
| 열쇠 출구 | 일반 출구와 다른 구역에 있다. 수색으로 열쇠를 얻었다면 일반 출구 요청 없이 이용할 수 있다 |
| 최종 마감 | 시설 시각 240칸에 붕괴. 그 순간 도착은 늦다 |

열쇠는 특정 수색 지점에서 얻는 대안이지 모든 출격에서 얻는 보장 수단이 아니다. 일반 출구를 놓친 뒤 열쇠까지 없으면 남은 시간을 수색에 걸어야 한다.

**플레이어의 선택:** 가동을 걸기 전에 주변을 정리할지, 게이지를 채우는 동안 은엄폐를 쓸지 고른다. 자리를 뜨면 가동이 취소되므로 "지금 걸어도 되는가"가 곧 판단이다. 너무 늦었다면 계약보다 탈출을 우선한다.
...
**다른 시스템과의 관계:** 해킹은 가동 게이지를 5칸에서 2칸까지 줄인다. 기동은 고지대 지름길로 출구까지의 방 수를 줄인다. 전투는 열린 시간을 소모할 수 있다.

**플레이 예시 · 약 30초:** 시각 140에 일반 출구에 도착했다. 해킹 0이라면 가동 1칸에 남은 4칸을 그 자리에서 채워 145에 열린다. 플레이어는 주변 순찰을 확인하고 그 5칸을 버틸 수 있는지 잰다. 자리를 뜨면 가동이 취소되므로 근처 수색은 가동 전에 끝내야 한다.
...
| 탈출구 가동 | 게이지를 다 채우면 문이 열린다 | 적이 신호를 향함, 자리를 뜨면 취소 | 가동을 걸 시점과 그동안의 방어를 고른다 |
...
| 높음 | 일반 출구는 150칸부터 새 가동 불가, 다른 선택은 수색 열쇠다 | 느린 조합이나 정보 계약이 목표보다 시간과 우연에 막힐 수 있다 | 계약×장비×시설별로 목표 작업·송출·귀환·개방까지 남는 시간 측정 |
...
| 중간 | 기동은 이동 시간을 바꾸지 않고 고지대 통로만 연다 | 지름길이 시간이 아니라 방 수를 줄이므로, 기동의 값이 시설 배치에 따라 크게 흔들릴 수 있다 | 기동 0과 3 빌드의 실제 귀환 홉수 차이 비교 |
...
‘몇 분짜리 게임’인지도 아직 확정하기 어렵다. 현재 지도는 네 구역 조합에 따라 180~248개 지점이고, 행동 시간은 실제 초와 다르다. 이동이 1칸으로 고정된 뒤 한 판의 칸은 거의 전부 작업과 대기에 쓰인다. 짧은 판이라는 과거 목표를 현재 체감 시간으로 단정하지 않는다.

### 9.4 첫 플레이테스트에서 볼 것

1. 계약·장비 조합별 목표 도착, 목표 확보, 탈출구 가동, 최종 도착 시각을 기록한다.
...
일반 출구는 150칸부터 새 가동을 받지 않는다. 이미 걸었다면 그 게이지와 개방 기회는 유지된다. 시설은 240칸에 붕괴한다. 현재 과부화는 무료 강화이며 카드 소음은 꺼져 있다. 영구 성장과 전투 이탈의 일관된 후속 결과는 아직 완성되지 않았다.
