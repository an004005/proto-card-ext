...
파밍은 실행 전 접근 방식을 고를 수 있다: `안전`(시간 +40, 소음 -1·최소 0 — 시설 붕괴 시간을 더 쓰는 대신 적 유입 위험을 낮춤), `보통`(표 값 그대로), `강행`(시간 -40·최소 20, 소음 +1·최대 3 — 빠르지만 적 유입·추적 위험 증가). 접근 방식은 시간/소음만 바꾸고 보상 자체는 바꾸지 않는다.

`대기` 명령은 제공하지 않는다. 무한 대기로 적/출구 상태를 공짜로 조작하는 것을 막는다.

기본 정찰을 완료하면 현재·인접 노드가 `실시간 정찰` 대상이 되며, 플레이어가 이동하거나 다른 정찰을 시작할 때까지 매 월드 틱마다 관측값을 갱신한다. 카메라 해킹 정찰은 이 대상을 교체하며 정해진 지속시간이 끝날 때까지 카메라 노드와 인접 노드를 갱신한다. UI는 실시간 정찰 대상을 파란 계열의 외곽선과 상태 문구로 표시한다.
...
- Mobility 3부터 `highGround` 특수 엣지를 통과할 수 있다. 그 미만에서는 이동 버튼을 비활성화하고 요구 수치와 현재 수치를 표시한다.
- 카메라와 접속 인터페이스는 노드별로 독립 배치되어 둘 중 하나만 또는 둘 다 존재할 수 있으며, 구역마다 각각 최소 하나를 보장한다.
- 접속 인터페이스의 카메라 해킹 사거리는 Hacking 1/2/3/4에서 1/2/3/4홉이다. 시간 100, Overload +6이며 카메라 감시 해제와 현재·인접 노드 실시간 정찰이 완료 후 300포인트 지속된다.
- 접속 인터페이스를 한 번 해킹하면, 그 뒤로는 홉 사거리와 무관하게 같은 구역 안의 모든 카메라를 조작할 수 있다(구역 전체 원격 접속). 직접 홉 사거리 규칙은 인터페이스를 아직 못 찾았거나 해킹하지 못했을 때의 기본 경로다.
- 카메라 파괴(Force 1+, 시간 100, 소음 2)는 해킹과 달리 영구적이며 재사용 대기가 없다 — 해킹은 300포인트 뒤 감시가 재개되지만 파괴는 그 카메라를 완전히 무력화한다.
- 작동 중인 카메라 노드에 Stealth 3 미만으로 진입하면 3홉 이내 위협은 해당 노드를 향해 추적한다. 발각 여부와 마지막 발각 위치·시각은 HUD에 명시한다.

**배터리 발전기**: `power`·`labs` 구역에 각 1개씩 배치되는 별도 장치. 무력화하지 않은 상태로 그 구역에서 전투에 진입하면, 그 전투의 모든 적이 전투 시작 시 갑옷 5를 얻는다. Hacking(직접/구역 원격 접속 규칙은 카메라와 동일, 시간 100·Overload +6) 또는 Force(반드시 발전기 노드에 있어야 함, 시간 100·소음 2)로 무력화하며, 한 번 무력화하면 영구적이다.
...
| 100 이상 | — | 아래 "100 초과 처리" 참고 — 더 이상 즉시 런 패배가 아니다 |

Overload는 0 이상 정수이며 자연 회복하지 않는다. 장착 임플란트가 만드는 Overload 바닥 아래로는 줄지 않는다. 무한 대기 악용을 막기 위해 시간을 보내 회복하는 방법은 없다.

**100 초과 처리**: 맵과 전투에서 서로 다른 방식으로 다룬다.

- **맵**: 100을 넘기게 되는 행동은 애초에 UI에서 비활성화한다(실행 전에 미리 계산해 막음) — 그래서 실제로는 맵에서 Overload가 100을 넘는 일이 없고, 넘어도 런이 끝나지 않는다. "멜트다운으로 즉시 런 패배" 규칙은 폐기됐다.
- **전투**: 카드/장비 사용으로 100을 넘기는 것 자체는 막지 않는다. 대신 초과분 10당(올림) 1장씩 저주 카드가 그 전투에만 임시로 덱에 삽입된다(전투가 끝나면 사라짐) — 매번 초과될 때마다 초과분에 비례해 반복 적용되는 페널티다.
...
- 맵에서 Overload 100을 넘기게 되는 행동은 버튼 자체가 비활성화되어 시도할 수 없다("100 초과 처리" 참고).
- 플레이어는 행동 미리보기에서 증가 후 단계를 본다.
...
`DecisionOption`은 `{ decisionId, riskScore: 0..100, reasonCodes }`를 가진다. 같은 결정을 후보별 위험 점수 오름차순으로 정렬하며 최저위험은 0~33, 차선은 34~66, 고위험은 67~100이다. `failureCause`는 `collapse > hp > reinforcement > pursuit > time > other` 우선순위의 단일 enum으로 기록해 원인이 겹치지 않게 한다(맵의 Overload 멜트다운 즉시 패배는 폐기되어 더 이상 실패 원인이 아니다 — "100 초과 처리" 참고).
...
| peak Overload | 55~80 | 90 이상은 마지막 수단(100을 넘기는 맵 행동은 애초에 막혀 있어 즉시 패배는 없음) |
