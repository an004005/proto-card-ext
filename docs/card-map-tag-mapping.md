...
- `traits` 중 현재 코드가 실제로 읽는 것은 `'healing'`과 `'override'` 둘뿐이다(맵에서 즉시 쓸 수 있는 소모품 판정 — 각각 회복류와 오버라이드 충전류). `deception`·`hack`·`perception`을 비롯한 나머지 태그는 **현재 소비처가 없다** — 분류용 메타데이터로만 남아 있으며, 어떤 규칙도 이 값을 보지 않는다. 맵의 기만·해킹·정찰 행동은 카드 태그가 아니라 장비 Capability(`facilityEquipmentCapabilities.js`)가 정한다.
