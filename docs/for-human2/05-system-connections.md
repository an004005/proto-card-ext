...
|---|---|---|---|
| 정찰 | 옆방 내용물, 위협 상세 | 4칸. 그 사이 위협 한 번 이동 | 파밍 가치와 조우 결과를 미리 계산할 수 있다 |
| 이동 | 위치 변경 | 통로 비용 2~13칸(기동으로 −4~+2). 은신 2 이하면 흔적 | 흔적이 발견되면 위협이 몰린다. 강한 흔적이면 경계도까지 |
| 보급 파밍 | 아이템 하나 즉시 | 5칸, 소음 1, 인벤토리 1칸 | 인벤토리가 차면 다음 파밍이 짐이 된다 |
| 확보 파밍 | 후보 3개 중 장비 하나 | 10~13칸, 소음 2~3, 그 방에 머묾 | 빌드가 바뀐다. 능력이 바뀌면 열리는 문과 조우 결과가 바뀐다 |
| 문 뜯기 | 통로 개방(영구) | 5칸, 소음 2. 위협도 그 길을 쓴다 | 짧은 길이 생기지만 추적 경로도 생긴다 |
| 문 해킹 | 통로 개방(영구), 카메라는 15칸 | 4칸, 조용. 모자라면 경계도 | 돌아올 때 카메라가 살아 있다 |
| 기습 전투 | 적 전원 기절 후 선공, 보상 3택 1 | 라운드당 3칸, HP·탄약·내구도, 시체 | 시체를 치울지(5칸) 두고 갈지. 증원이 자리를 채운다 |
| 무시 | 0칸 | 위협이 그 자리에 남는다 | 돌아올 길이 막힐 수 있다 |
| 회피 | 추적 끊김 | 1칸 | 다음 행동 뒤 재판정 |
| 계약 목표 작업 | 계약 완료 조건 진행 | 6~9칸, **봉쇄 시작** | 이후 모든 시간 계산이 다시 짜인다. 파밍은 위험해지고 출구가 급해진다 |
| 수습 | 경계도 하락·이동·정지, 흔적 제거 | 4~8칸, 일부는 소음 3 | 그 구역을 다시 지날 수 있게 된다. 옆 구역이 위험해질 수 있다 |
| 탈출 요청 | 개방 대기 시작 | 3칸 + 5~15칸. 위협이 출구로 모인다 | 대기 중 조우를 어떻게 넘길까 |

## 여섯 덩어리의 관계

### 전투 ↔ 탐색

- 같은 시계를 쓴다. 전투 한 라운드가 순찰 위협 한 걸음과 거의 같다(3칸 대 5칸).
- 조우 판정이 둘 사이의 문이다. 탐색 쪽 결정(어디서 마주칠까, 카메라를 먼저 껐나, 은엄폐가 있나)이 전투의 시작 조건(선공, 기절, 추가 3칸)을 정한다.
...
- 이 게임의 "성장"은 런 안에서 확보 대상으로 얻은 장비를 끼우는 것이다. 장비를 바꾸면 덱과 능력이 함께 바뀐다(맵에서 건당 3칸).
...
- 표준 출구 폐쇄 300칸이 실질 마감이다. 열쇠는 보험이지만 우연히 나온다.
- 탈출 요청이 위협을 출구로 모으고, 개방 대기 길이는 해킹 능력이 정한다.
...
| 은신 | 흔적 남기지 않음 | 조우 우위, 카메라 안 걸림(3 이상) | 기습 선공 | 대기 중 조우 |
| 해킹 | 전자문, 카메라, 인터페이스, 발전기 원격 | 통제실 장악(유일한 실제 하락) | 발전기 무력화로 적 갑옷 제거 | 개방 대기 15→5칸 |
| 기동 | 이동 −1~−4칸, 고지대 | — | 이탈 +1 시작 | 출구까지 시간 |
