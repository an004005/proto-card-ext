...
- 런 중에 Capability를 바꾸는 방법은 **장비 교체**(건당 2칸)와 **HP 회복**이다 — HP가 줄면 아래 부상 페널티가 걸린다.
...
### 부상 페널티 (`INJURY_PENALTY_STEPS`, ADR-0093)

HP 비율이 문턱 아래로 내려가면 **여섯 Capability가 전부** 그만큼 깎인다. 문턱은 내림차순으로 읽고 처음 걸리는 한 칸만 쓴다(누적이 아니다). 상태를 저장하지 않고 매번 HP에서 계산하므로 붕대로 HP가 돌아오면 그 자리에서 풀린다.

| HP 비율 | 보정 |
|---|---|
| 50% 이하 | 전 Capability −1 |
| 25% 이하 | 전 Capability −2 |

깎은 뒤에도 −2~4로 자른다. 오버라이드 칩을 쓰는 동안만은 예외로 여섯 값이 전부 상한이다(ADR-0086) — 부상도 그 약속을 깎지 않는다. 그래서 맵에서 쓰는 회복 소모품이 전투 대비가 아니라 **침투 자원**이 된다.
...
| 특수 엣지 개방 | Force 또는 Hacking | 엣지가 정함(기본 1) | 3 / 2 | 2 / 0 |
| 접속 인터페이스 해킹 | Hacking | 1 | 3 | 0 |
| 카메라 해킹 | Hacking | 1 | 3 | 0 |
| 카메라 파괴 | Force | 1 | 3 | 2 |
| 카메라 저격 | Perception | 2 | 2 | 2 |
| 발전기 해킹 무력화 | Hacking | 1 | 3 | 0 |
| 발전기 파괴 | Force | 1 | 3 | 2 |
| 통제실 장악 | Hacking | 1 | 4 | 0 |
| 물건 확보(회수 계약) | Stealth 또는 Mobility | 1 | 3 | 0 |
| 폭약 설치 | Force | 1 | 5 | 0 |
| 데이터 확보 | Hacking | 1 | 3 | 0 |
| 데이터 송출 | Hacking | 1 | 3 | 0 |
| 흔적 정리 | Perception | 1 | 2~4(전용표) | 0 |
| 전원 차단 | Force | 1 | 3 | 3 |
| 가짜 목표 송출 | Deception | 1 | 4 | 0 |
| 가짜 소음 | Deception | 1 | 2 | 0 |
...
- 층계나 접근 보정이 걸린 행동은 툴팁에 `문 해킹 4칸 = 기본 2 + 능력 2`처럼 분해를 낸다(`describeForecast`).
- 하한에 잘렸으면 그 행동의 실제 하한을 적는다(층계가 걸린 행동은 `CAPABILITY_MIN_TIME` = 1칸, 접근 모드만 걸린 행동은 `APPROACH_MIN_TIME` = 1칸).
