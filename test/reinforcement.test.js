...
test('a threat that walks onto a corpse reports it: the sector alert rises, an investigation is drawn there, and the corpse is gone', () => {
  ...
  assert.equal(after.sectorAlerts[sectorId].pressure, ALERT_PRESSURE.corpseFound, '시체 발견은 그 구역 경계 게이지를 올린다');
...
test('a weak trace only draws an investigation; only a strong trace raises the alert', () => {
  ...
  assert.equal(afterWeak.sectorAlerts[sectorId].pressure, 0, '약한 흔적은 경계 게이지를 올리지 않는다');
  ...
  assert.equal(afterStrong.sectorAlerts[sectorId].pressure, ALERT_PRESSURE.strongTraceFound, '강한 흔적은 경계 게이지를 올린다');
});
...
test('a rising alert pulls that sector’s next reinforcement forward', () => {
  ...
  const escalated = {
    ...base,
    threats,
    sectorAlerts: { ...base.sectorAlerts, [sectorId]: { level: 1, pressure: 0, resolvedEventIds: ['cause'] } },
  };
