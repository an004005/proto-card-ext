...
test('사거리 안의 카메라는 작업이 끝나면 영구히 부서지고, 소음은 쏜 자리에 흔적은 카메라 자리에 남는다', () => {
  ...
  const after = useFieldEquipment(run, INSTANCE, SNIPER, camera.id, { effectivePerception: 2, usableAmmo: 10 });
...
test('사거리 밖과 잠긴 통로 너머의 카메라는 쏠 수 없다', () => {
  ...
  assert.throws(
    () => useFieldEquipment(run, INSTANCE, SNIPER, farCamera.id, { effectivePerception: 2, usableAmmo: 10 }),
    RuleViolation,
  );
  ...
  assert.throws(
    () => useFieldEquipment(sealed, INSTANCE, SNIPER, camera.id, { effectivePerception: 2, usableAmmo: 10 }),
    RuleViolation,
    '잠긴 통로 너머는 시야가 없다',
  );
});
...
test('Perception 층계는 시간으로만 받는다 — 1은 무리(+2), -1은 불가', () => {
  ...
  const strained = useFieldEquipment(run, INSTANCE, SNIPER, camera.id, { effectivePerception: 1, usableAmmo: 10 });
  ...
  const severe = useFieldEquipment(run, INSTANCE, SNIPER, camera.id, { effectivePerception: 0, usableAmmo: 10 });
  assert.equal(severe.time, run.time + CAMERA_SNIPE_TIME + CAPABILITY_STEP_TIME_DELTA.severe);

  // Perception -1부터는 아예 겨눌 수 없다.
  assert.throws(
    () => useFieldEquipment(run, INSTANCE, SNIPER, camera.id, { effectivePerception: -1, usableAmmo: 10 }),
    RuleViolation,
  );
});
...
test('탄이 없으면 쏠 수 없고, 이미 부순 카메라는 다시 쏠 수 없다', () => {
  const { run, camera } = runWithCameraInSight();
  assert.throws(
    () => useFieldEquipment(run, INSTANCE, SNIPER, camera.id, { effectivePerception: 2, usableAmmo: 0 }),
    RuleViolation,
  );

  const destroyed = { ...run, disabledCameraIds: [camera.id] };
  assert.throws(
    () => useFieldEquipment(destroyed, INSTANCE, SNIPER, camera.id, { effectivePerception: 2, usableAmmo: 10 }),
    RuleViolation,
  );
});
