// 맵 시계 — 현재 시각, 붕괴까지, 표준 출구 A의 지금 상태. 맵 화면과 전투 화면이 **같은
...
export function MapClock({ run, countdowns = runCountdowns(run), compact = false }) {
  ...
  const tip = `현재 시각 ${run.time} / ${RUN_COLLAPSE_TIME}칸. ${RUN_COLLAPSE_TIME}칸에 도달하면 즉시 런이 종료됩니다(탈출 실패). 표준 출구 A는 정해진 시각 뒤로 새 가동을 받지 않습니다 — 이미 시작된 가동 게이지와 열린 창은 끝까지 갑니다. 봉쇄는 이 시각을 앞당기지 않습니다.`;
  return html`
