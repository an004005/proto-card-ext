...
// 로비 방이 1층 복도를 둘러싸는 고리의 반지름(복도 한 칸 SPACING 기준). 탑은 위아래로 길어서
// 정규화하면 가로 폭이 크게 줄어든다 — 기본 간격 그대로 두면 로비 노드들이 최소 간격에 딱
// 붙은 덩어리로 보인다. 그렇다고 겹겹이 멀리 밀어내면(예전의 거리 배수 방식) 바깥 겹의 방이
// 층 간격의 네 배 넘게 떨어져, 평범한 통로가 도면을 가로지르는 긴 줄이 된다(ADR-0097).
// 그래서 로비 방은 전부 **같은 반지름의 고리** 위에 고르게 놓는다: 방마다 거리가 같고, 층
// 간격(1.5)의 두 배를 넘지 않으며, 고리 위 이웃 간격도 최소 간격보다 넓다.
export const TOWER_LOBBY_RING_RADIUS = 2.0;
...
// 특징(features)이 없는 평범한 통로는 "옆방으로 난 문"이어야 한다. 평면도를 가로질러 멀리
// 뻗거나 여러 칸을 건너뛰는 지름길은 언제나 특수 통로다 — 지름길에는 대가가 붙어야 한다.
// 판정은 생성 좌표에서 두 가지로 한다(facilityGraph.js findPlainEdgeViolations).

// (a) 길이: 그 구역의 평범한 통로 길이 **중앙값**의 이 배수를 넘으면 위반이다. 중앙값은 방을
// 복도에 매단 짧은 문이 다수라 복도 간격보다 낮게 잡힌다 — 배치 원형별 실측에서 대공간의 홀
// 고리는 중앙값의 1.6배, 탑의 로비 고리는 2.4배까지 올라가지만 둘 다 실제로 맞닿은 뼈대다.
// 2.5가 그 뼈대를 건드리지 않으면서 방사형 바깥 고리(최대 11배)처럼 도면을 가로지르는 줄만
// 집는 값이다.
export const PLAIN_EDGE_MAX_LENGTH_FACTOR = 2.5;

// (b) 사이에 낀 방: 두 끝을 지름으로 하는 원(Gabriel 원) 안에 같은 구역의 다른 노드가 이 수보다
// 많이 들어 있으면 위반이다 — 사이에 방이 여럿 있는데 그 위를 건너뛰어 문을 낸 셈이다. 길이와
// 달리 축척을 타지 않아서, 노드가 성기게 놓인 구역과 빽빽한 구역에 같은 기준이 선다. 셋까지
// 봐주는 것은 복도에 매단 방이 이웃 복도끼리의 원 안에 흔히 걸리기 때문이다(격자 허브·대공간에서
// 실측 2~3개). 넷 이상은 방사형 바깥 고리처럼 도면을 실제로 가로지르는 줄에서만 나온다.
//
// 처음에는 이것을 "그 통로를 뺀 평범한 부분그래프에서 두 끝의 홉 거리"로 재려 했지만 그 수치는
// 지름길을 가려내지 못한다 — 길이 k인 고리 위에서는 **모든** 엣지의 홉 거리가 k−1이라, 고리를
// 닫은 지름길과 그 지름길이 건너뛴 복도가 똑같이 걸린다. 건너뛴 쪽을 가려내는 것은 결국
// 기하학이라, 길이와 "사이에 무엇이 끼어 있나" 둘로 판정한다.
export const PLAIN_EDGE_MAX_BLOCKING_NODES = 3;

// 화면 배치(components/mapLayout.js)에서도 같은 규칙을 지킨다 — 생성 좌표에서 짧던 통로가
// 화면 정리 과정에서 길어지면 플레이어에게는 여전히 "가로지르는 긴 줄"이다. 구역별 화면상
// 평범한 통로 길이 중앙값의 이 배수를 상한으로 당긴다.
export const PLAIN_EDGE_SCREEN_MAX_LENGTH_FACTOR = 2.0;
...
// 마감은 언제나 "시작점에서 가장 먼 출구(A)까지의 실측 중앙값"에 매여 있다. 배수는 세 배로
// 늘렸다(ADR-0087) — 구역 규모가 두 배가 되면서(ADR-0088) 한 런에서 "무엇을 더 챙길까"를 고를
// 폭을 넓히기 위해서다. 배수가 재는 것은 "몇 칸인가"가 아니라 "가장 먼 출구까지 왕복하면서
// 계약을 끝낼 수 있는가"이고, 그 위에 시설을 실제로 뒤질 여유를 얹은 값이다. 현재 실측
// 중앙값과의 배율은 `npm run balance:map`이 출력한다.
export const RUN_COLLAPSE_TIME = 240; // §2.3 t>=RUN_COLLAPSE_TIME 붕괴, 다른 모든 사건보다 우선.

// 표준 출구는 A 하나뿐이다(ADR-0083). 폐쇄는 "가동을 시작하는 시각"만 본다.
export const EXIT_A_DISABLED_AT = 150;
